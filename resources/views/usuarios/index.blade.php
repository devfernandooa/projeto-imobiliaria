                    ...
                    @if (count($usuarios) > 0)
                        @foreach ($usuarios as $usuario)
                            <tr>
                                ...
                                <td>{{ $usuario->email }}</td>
                                <td>{{ $usuario->cpf }}</td>
                                <td><span class="badge bg-secondary">{{ ucfirst($usuario->tipo_usuario) }}</span></td> {{-- Badge para tipo --}}
                                ...
                                <td class="text-center">
                                    {{-- Botões de Ação --}}
                                    <a href="#" class="btn btn-info btn-sm me-1" title="Ver Detalhes">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="#" class="btn btn-warning btn-sm me-1" title="Editar Usuário">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="#" method="POST" class="d-inline"> {{-- Usar form para DELETE --}}
                                        @csrf {{-- Proteção CSRF --}}
                                        @method('DELETE') {{-- Método HTTP DELETE --}}
                                        <button type="submit" class="btn btn-danger btn-sm" title="Excluir Usuário" onclick="return confirm('Tem certeza que deseja excluir este usuário?');">
